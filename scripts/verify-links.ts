...
async function verifyLinks() {
  ...
  const linksToCheck = Array.from(
    new Set(Object.values(SOURCES).map((source) => source.url))
  )
  ...
  for (const url of linksToCheck) {
    ...
    try {
      ...
      if (
        ...
        (url.includes('.nic.in') || url.includes('.gov.in'))
      ) {
        console.warn(
          `⚠️ Warning: Failed to fetch ${url} via Node.js (${error.message}), but domain is allowed.`
        )
      } else {
