...
test.describe('Civic Flows', () => {
  for (const flow of FLOWS) {
    test(`should load flow page: ${flow}`, async ({ page }) => {
      ...
      await expect(
        page.getByLabel('Breadcrumb').getByRole('link', { name: 'Guide' })
      ).toBeVisible()
    })
  }
  ...
  test('should load timeline page', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { level: 1, name: /Election Timelines/i })
    ).toBeVisible()
    await expect(
      page.getByRole('heading', {
        level: 2,
        name: /Election Summary|No timeline updates available/i,
      })
    ).toBeVisible()
  })
  ...
  test('should load learn page', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { level: 1, name: /Learn the Process/i })
    ).toBeVisible()
  })
  ...
  test('homepage should load and contain all task links', async ({ page }) => {
    ...
    for (const flow of FLOWS) {
      await expect(
        page.locator(`a[href="/flow/${flow}"]`).first()
      ).toBeAttached()
    }
  })
  ...
  test('timeline page passes the accessibility smoke check', async ({
    page,
  }) => {
    await page.goto('/timeline')
    await expectNoCriticalA11yIssues(page)
