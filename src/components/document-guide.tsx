...
export default function DocumentGuide({ documents, documentGuideLink }: Props) {
  ...
  if (documentGuideLink) {
    return (
      <Link
        href={documentGuideLink}
        className="mt-8 flex items-center gap-3 text-primary font-bold text-sm hover:underline"
      >
    ...
    <details className="group mt-8">
      ...
      <div className="mt-3 space-y-4 border border-gray-100 rounded-2xl p-6 bg-white">
              ...
              {doc.examples && doc.examples.length > 0 && (
                <ul className="space-y-1 mt-1">
                  {doc.examples.map((ex) => (
                    ...
                    >
                      <span
                        className="w-1 h-1 rounded-full bg-text-light mt-2 shrink-0"
                        aria-hidden="true"
                      />
                      {ex}
                    </li>
        ...
        <p className="text-xs text-text-light pt-2 border-t border-gray-100 mt-4">
          Accepted documents are subject to ECI guidelines. When in doubt, call
          Voter Helpline{' '}
          <a
            href="tel:1950"
