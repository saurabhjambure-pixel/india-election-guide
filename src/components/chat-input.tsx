...
export default function ChatInput() {
  ...
  async function submitQuery(
    msg: string,
    context?: { original: string; question: string }
  ) {
    if (!msg) return
    ...
    try {
      ...
      if (
        data.recommended_flow_id &&
        !data.needs_clarification &&
        data.intent !== 'out_of_scope'
      ) {
        logCustomEvent('task_selected', {
          flow_id: data.recommended_flow_id,
          origin: 'ask-guide',
          intent: data.intent,
        })
  ...
  return (
    <div className="w-full">
      <form
        ...
        role="search"
        aria-label="Ask about voter services"
      >
        <label htmlFor="chat-input" className="sr-only">
          {clarificationContext
            ? clarificationContext.question
            : 'Ask a voter question'}
        </label>
        <input
          ref={inputRef}
          id="chat-input"
          ...
          placeholder={
            clarificationContext
              ? 'Type your answer…'
              : 'e.g. How do I correct my name on my Voter ID?'
          }
          value={value}
      ...
      {clarificationContext && (
        <div
          className="mt-6 p-6 rounded-2xl note-box"
          role="status"
          aria-live="polite"
        >
          <p className="eyebrow mb-2">One more thing</p>
          <p
            className="font-bold text-lg leading-snug serif"
            style={{ color: 'var(--ink)' }}
          >
            {clarificationContext.question}
          </p>
      ...
      {result?.intent === 'direct_answer' && result.direct_answer && (
        <div
          className="mt-8 p-8 rounded-3xl premium-card"
          role="status"
          aria-live="polite"
        >
          ...
          <p
            className="text-xl leading-relaxed serif"
            style={{ color: 'var(--ink)' }}
          >
          ...
          <div className="mt-6 pt-6 border-t border-ink/5">
            <p className="text-xs opacity-50">
              Was this helpful? Your feedback helps the guide improve.
            </p>
          </div>
        </div>
      )}
