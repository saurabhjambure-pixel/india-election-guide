...
function buildTimelineFallback(timelines: TimelineRecord[]): TimelineSummary {
  if (timelines.length === 0) {
    return {
      ...
      summary:
        'No verified election schedule entries are available right now. Check the official ECI schedule for the latest notifications.',
      highlights: [
  ...
  const pendingEvents = timelines.filter(
    (item) =>
      item.status === 'Pending Announcement' || item.status === 'Date TBC'
  )
  const nextEvent =
    liveEvents[0] ?? upcomingEvents[0] ?? pendingEvents[0] ?? timelines[0]
  ...
  return {
    ...
    highlights: timelines
      .slice(0, 3)
      .map((item) => `${item.state}: ${item.event} — ${item.date}`),
    sourceLabel: 'Fallback',
  }
}
...
export async function generateTimelineSummary(
  ...
  try {
    const result = await model.generateContent({
      contents: [
        { role: 'user', parts: [{ text: buildTimelinePrompt(timelines) }] },
      ],
      generationConfig: {
